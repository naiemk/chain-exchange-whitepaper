...
- Proof of burn futures: The proof of burn future is the only future that does not involve a specific time. It involves two transactions, one to claim *Fe(X)*s in the form of transferring funds from a null wallet to the *w_p'* (where *p'* is a temporary wallet), and another to transfer funds from *w_p'* to *w_new* where *w_new* is a new wallet. The wallet *w_p'* is created to match an external wallet *w_X_p'* and is of a temporary nature. The second transaction can be validated within the network by nodes that can not read into the *X* network.
- Edge futures: In addition to proof of burn, edge futures provide means of transacting between *Ferrum* network and an external network *X*. An edge future is a transaction that happens between *Alice* and *Bob* and in the process they exchange *Fe(X)* with equivalent amount of funds from *X* at future time *t_f*. Let *w_X_a* be Alice's wallet in *X* and *w_X_b* be Bob's wallet in *X*. Also *w_a* is Alice's wallet in *Ferrum*. Alice already has funds in her wallet *w_a*. She will create a special temporary wallet *w_a'* plus another permanent wallet *w_a''*. Bob will also create a wallet *w_b*. Alice will send a special transaction *T_a,a',a'',b*: *w_a* -> *w_a'*with a reference time *t_f*. *T_a,a',a'',b* references *w_a''* as the *rollback address*, and *w_b* as the *target address*. *T_a,a',a'',b* will also have a condition *w_X_b* -> *w_X_a* for a set amount. Note that Alice and Bob have already agreed on the external wallets, the amount, and the future time *t_f*. *t_f* could also be influence by the plugin to ensure enough time is passed for the transaction *w_X_b* -> *w_X_a* in network *X* be completed irreversibly. Up until *t_f+buffer* is reached, funds in *w_a'* are locked. After *t_f*, *T_a,a',a'',b* will be read as *T_a',b* if the condition is fulfilled, or *T_a',a''* if the condition is not fulfilled, hence the transaction is rolled back. A node that does not have the relevant plugin for *X* cannot validate an edge transaction, so it can assume that either *T_a',a''* or *T_a',b* are valid. However, Alice or Bob can only use their *Fe_X* if the node accepting their funds has the appropriate plugin and can validate the Edge transaction.
